# Claude Code Context — AcadiaLogic.Dutchie.Library
...
```
AcadiaLogic.Dutchie.Library.slnx        ← note: .slnx not .sln
```

Build command:
```sh
dotnet build /Users/jaradhuffman/Repos/AcadiaLogic.Dutchie.Library/AcadiaLogic.Dutchie.Library.slnx
```
...
|---|---|---|
| `src/AcadiaLogic.Dutchie.Library` | `AcadiaLogic.Dutchie` | Typed HTTP client for the Dutchie POS REST API |
| `src/AcadiaLogic.Dutchie.Integration` | `AcadiaLogic.Dutchie.Integration` | ERP-neutral abstractions, sync pipelines, state store |
| `src/AcadiaLogic.Dutchie.Intacct` | `AcadiaLogic.Dutchie.Intacct` | Sage Intacct SDK connector + Platform App config provider |
| `src/AcadiaLogic.Dutchie.Worker` | `AcadiaLogic.Dutchie.Worker` | Generic Host worker; entry point |
...
| File | Role |
|---|---|
| `platform-app/DutchieIntegration.xml` | Intacct Platform Application — deploy once per company |
